...
int mysql_dao::reg_user(const std::string& name, const std::string& email, const std::string& pwd)
{
	...
	try {
	    ...
	    unique_ptr<sql::Statement> stmt_result(con->_con->createStatement());
	    unique_ptr<sql::ResultSet> res(stmt_result->executeQuery("SELECT @result AS result"));
	    if (res->next()) {
...
bool mysql_dao::check_pwd(const std::string& email, const std::string& pwd, UserInfo& userInfo) {
	auto con = pool_->get_conn();
	if (con == nullptr) {
		return false;
	}
